...
export function applyFocusPolicy(logic) {
    ...
    if (trapFocus && !TRAP_FOCUS_ROLES.has(role)) {
        throw new PolicyViolationError('FocusPolicy', `Component "${logic.name}" sets trapFocus=true but role="${role}" does not warrant a focus trap. Use role: dialog, alertdialog, menu, or listbox.`);
    }
    if (!focusVisible) {
        console.warn(`[OBIX FocusPolicy] Component "${logic.name}" has focusVisible=false. This may violate WCAG 2.4.11 Focus Appearance.`);
    }
    return logic;
}
