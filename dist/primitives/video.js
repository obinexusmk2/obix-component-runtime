...
function renderVideo(state) {
    const tracks = state.tracks
        .map((t) => `<track src="${t.src}" kind="${t.kind}" srclang="${t.srclang}" label="${t.label}"${t.default ? ' default' : ''}>`)
        .join('\n  ');
...
export function createVideo(config) {
    const logic = {
        name: 'ObixVideo',
        state: {
            src: config.src,
        ...
        actions: {
            ...
            setSrc: (_state, src) => ({ src: String(src), playing: false }),
        },
        render: renderVideo,
        aria: {
            role: 'region',
