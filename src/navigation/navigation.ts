...
function renderNavigation(state: NavigationState): string {
...
export function createNavigation(config: NavigationConfig): ComponentLogicWithAccessibility<NavigationState> {
  const menuId = (config.id ?? `obix-nav-${Math.random().toString(36).slice(2, 8)}`) + '-menu';
  const logic: ComponentLogicWithAccessibility<NavigationState> = {
    name: 'ObixNavigation',
    state: {
      items: config.items,
    ...
    actions: {
      setActive: (_state, id: unknown) => ({ activeId: String(id) }),
