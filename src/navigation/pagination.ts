...
function renderPagination(state: PaginationState): string {
...
export function createPagination(config: PaginationConfig): ComponentLogicWithAccessibility<PaginationState> {
  const logic: ComponentLogicWithAccessibility<PaginationState> = {
    name: 'ObixPagination',
    state: {
    ...
    actions: {
      goToPage: (state, page: unknown) => {
        const p = Number(page);
        if (p < 1 || p > state.totalPages) return state;
