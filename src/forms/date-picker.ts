...
function renderDatePicker(state: DatePickerState): string {
...
export function createDatePicker(config: DatePickerConfig): ComponentLogicWithAccessibility<DatePickerState> {
  const id = config.id ?? `obix-datepicker-${Math.random().toString(36).slice(2, 8)}`;
  const now = new Date();
  const logic: ComponentLogicWithAccessibility<DatePickerState> = {
    name: 'ObixDatePicker',
    state: {
    ...
    actions: {
      ...
      selectDate: (_state, date: unknown) => ({ value: String(date), open: false }),
